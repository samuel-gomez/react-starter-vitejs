...
describe('NumberInput', () => {
  ...
  `('Affichage du playground NumberInput', async ({ profil }) => {
    ...
    expectLink({ name: 'Guidelines', href: 'https://axafrance.github.io/design-system/molecules/form-text/' });
    ...
    expectLink({ name: 'Storybook', href: 'https://axafrance.github.io/react-toolkit/latest/storybook/?path=/story/form-input-number--number' });
    ...
    expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toHaveAttribute('value', '5');
    expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toHaveAttribute('placeholder', 'Ex: Samuel');

    // And un message "Enter your number" est visible avec la classe : "af-form__help"
    expect(scopePreview.getByText(/Enter your number/)).toHaveClass('af-form__help');
    ...
    expect(scopeCode.queryAllByText(/NumberInput/).length).toEqual(2);
  });
  ...
  `(
    ...
    async ({ profil, value, placeholder, className, modifier, expectedClass, expectedId, expectedPlaceholder, expectedValue }) => {
      ...
      expect(scopeEditor.getByRole('textbox', { name: /value/ })).toHaveValue('5');
      ...
      const scopePreview = setScope(SCOPE_PREVIEW);
      const inputDemo = scopePreview.getByRole(ROLE_NUMBER, { name: /name-field/ });
  ...
  `('Activation/desactivation du champ input', async ({ profil }) => {
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toBeDisabled());
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toBeEnabled());
  });
  ...
  `('Requis/optionnel du champ input', async ({ profil }) => {
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toBeRequired());
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).not.toBeRequired());
  });
  ...
  `('ReadOnly/editable du champ input', async ({ profil }) => {
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).toHaveAttribute('readOnly'));
    ...
    await waitFor(() => expect(scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ })).not.toHaveAttribute('readOnly'));
  });
  ...
  `('Affichage/masquage du champ input', async ({ profil }) => {
    ...
    const inputDemo = scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ });
    ...
    await waitFor(() => expect(scopePreview.queryByRole(ROLE_NUMBER, { name: /number-field/ })).not.toBeInTheDocument());
  ...
  `(
    ...
    async ({ profil, message, type, expectedWrapperInputClass, expectedDisplayMessage, expectedDisplayMessageClass }) => {
      ...
      const scopeEditor = setScope(SCOPE_EDITOR);
      expect(scopeEditor.getByRole('textbox', { name: /message/ })).toHaveValue('');
      ...
      const scopePreview = setScope(SCOPE_PREVIEW);
      const inputDemo = await scopePreview.getByRole(ROLE_NUMBER, { name: /number-field/ });
  ...
  `('Modification du label', async ({ profil }) => {
    ...
    expect(dialog.getByText(/Saisir la value de label/)).toBeInTheDocument();

    // And un champ texte "jsx-code-editor" est visible avec la valeur "My number"
    expect(dialog.getByLabelText('Jsx Code Editor Input')).toHaveValue('My number');
    ...
    const scopePreview = setScope(SCOPE_PREVIEW);
    await waitFor(() => expect(scopePreview.getByLabelText(/sample title/)).toBeInTheDocument());
  });
});
