...
describe('setSort', () => {
  const onSort = vi.fn().mockReturnValue('onSortResult');
  const toggleSortingFn = vi.fn().mockReturnValue('sortByResult');
  const defaultProps = {
    field: '',
...
const onSort = vi.fn().mockReturnValue('ordering');
const ThSortableCmpt = vi.fn().mockImplementation(({ children }: { children: ReactNode }) => (
  <>
    ThSortableCmpt
    {children}
  </>
...
const setClassModifierActiveFn = vi.fn();
